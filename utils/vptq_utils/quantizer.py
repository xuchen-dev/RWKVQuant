...
class QuantizationArguments:
    vector_lens: List[int] = field(default_factory=lambda: [3584, 3584])
    num_centroids: List[int] = field(default_factory=lambda: [8, 8])
    num_res_centroids: List[int] = field(default_factory=lambda: [-1, -1])
    npercent: float = field(default=10)
...
class NPVectorQuantizer:
    ...
    def __init__(
        ...
        self.vector_lens = vector_lens # [-1, 1]
        self.num_centroids = num_centroids # [-1, 1]
        self.num_res_centroids = num_res_centroids
        self.npercent = npercent # 0
        ...
        self.iter = iter   # 100
        self.tol = tol     # 1e-05
        self.kmeans_seed = kmeans_seed # 0

        # vector_len
        self.outlier_vector_len = self.vector_lens[0] # -1
        self.vector_len = self.vector_lens[1] # 1
        ...
        self.kmeans_mode = kmeans_mode # 'hessian'
        # self.kmeans_alpha = kmeans_alpha

        self.enable_norm = enable_norm # false
        self.norm_dim = norm_dim # 0
    ...
    def set_num_centroids(self, num_centroids):
        self.num_centroids = num_centroids

    def configure(self, wbit=None, perchannel=None, sym=None, mse=None):
        return
    ...
    def init_centroids_indices(self, data, weights=None):
        ...
        for idx, (i, j) in enumerate(self.get_index_list(data)):
            ...
            if num_centroids == -1:  # Do not quantize, keep original data
                ...
                _kmeans.fit(sub_vectors, sample_weight=vector_weights) # (3072, 8)  (3072,)
    ...
    def quantize_vector(self, data, index):
        ...
        if self.centroids[cidx] is None:
            ...
            dist = torch.cdist(data.float(), self.centroids[cidx].float()) # [8,8]  [4096, 8]
