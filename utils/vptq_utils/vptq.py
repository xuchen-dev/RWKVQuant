...
class VPTQ:
    ...
    def __init__(
        ...
        # preprocess
        self.layer = layer
        self.weight = self.layer.weight.data
        self.qweight = torch.zeros_like(self.weight)
    ...
    def add_batch(self, inp, out):  # 更新海森矩阵
        if len(inp.shape) == 2:
            inp = inp.unsqueeze(0)
        tmp = inp.shape[0]
        ...
        self.H *= self.nsamples / (self.nsamples + tmp)  # 更新比例
        self.nsamples += tmp
        inp = math.sqrt(2 / self.nsamples) * inp.float()  # 
        self.H += inp.matmul(inp.t())

    # vector quant
    def fasterquant(self, init_centroids=True, **kwargs):
        self.init_centroids = init_centroids

        # step 0: preprocess weight and hessian
        weight = self.weight.clone().float().to(self.dev)
        hessian = self.H.clone().to(self.dev)
        # inv_hessian = self.inv_hessian.clone().to('cpu')

        dead = torch.diag(hessian) == 0 # 海森矩阵是0 那么
        if dead.sum() != 0:
            print("has dead hession matrix")
        hessian[dead, dead] = 1

        damp = 0.1 * torch.mean(torch.diag(hessian)) # 0.01 * 17.2049
        diag = torch.arange(self.columns, device=self.dev)
        hessian[diag, diag] += damp  # 这一步为什么 ？  直接加上了这个惩罚项
        H = torch.linalg.cholesky(hessian)
        ...
        if hasattr(self.quantizer, "enable_perm") and self.quantizer.enable_perm is not None:
        ...
        if hasattr(self.quantizer, "enable_perm") and self.quantizer.enable_perm:
        ...
        self.layer.weight.data = qweight.reshape(self.layer.weight.shape).to(self.layer.weight.data.dtype)
    ...
    def vptq(self, weight, hessian, enable_residual=False, inv_hessian=None):
        ...
        if inv_hessian is None:
            ...
            hessian = torch.linalg.cholesky(hessian, upper=True).to(torch.float32)
            inv_hessian = hessian.to(torch.float32)
        else:
            inv_hessian = inv_hessian.to(torch.float32)
    ...
    def init_qlinear(self, dev):
        quantizer = self.quantizer
        perm = self.quantizer.perm

        weight = self.weight.clone().detach().to(dev)
        ...
        indices = self.quantizer.indices

        # res centroid
        # num_res_centroids = quantizer.num_res_centroids
        res_centroids = self.quantizer.res_centroids
        # res_centroids = quantizer.res_centroids[1]
        res_indices = self.quantizer.res_indices
