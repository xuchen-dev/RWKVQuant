...
class QuantizerBase(nn.Module):
    ...
    def extra_repr(self):
        return "n_bits={}, per_channel={}, is_initalized={}".format(self.n_bits, self.per_channel, self.is_initialized)
...
class AsymmetricUniformQuantizer(QuantizerBase):
    ...
    def to_integer_forward(self, x_float):
        ...
        if self.grad_scaling:
            ...
            zero_point = self.zero_point if self.symmetric else scale_grad_func(self.zero_point, grad_scale)
        else:
            scale = self.scale
    ...
    def _tensorize_min_max(self, x_min, x_max):
        ...
        if x_min.dim() > 0 and len(x_min) > 1 and not self.per_channel:
            ...
            raise ValueError("x_min and x_max must be a float or 1-D Tensor" " for per-tensor quantization (per_channel=False)")
