...
import transformers
import tqdm, math
from quant.hadamard_utils import random_hadamard_matrix, apply_exact_had_to_linear, is_pow2
...
def fuse_layer_norms(model):  # fuse all layernorms's weight into the adjacent linear blocks

    model_type = model_utils.get_model_type(model)

    kwargs = {"model": model, "model_type": model_type}

    # Embedding fusion # sub mean-value
    ...
    for layer in layers:
        ...
        # fuse the input layernorms into the linear layers 将layernorm的权重融入到linear中
    ...
    # fuse last layernorm's weight into the output projection
...
def rotate_attention_output(layer, Q, model_type) -> None:
    ...
    dtype = W.weight.data.dtype
    W_ = W.weight.data.to(device=utils.DEV, dtype=torch.float64)
    W.weight.data = torch.matmul(Q.T, W_).to(device="cpu", dtype=dtype)  # Q^T @  W_out
    if W.bias is not None:
        b = W.bias.data.to(device=utils.DEV, dtype=torch.float64)
        W.bias.data = torch.matmul(Q.T, b).to(device="cpu", dtype=dtype)  # Q^T @  W_out_bias
...
def rotate_ov_proj(layer, model_type, head_num, head_dim):
    ...
    apply_exact_had_to_linear(v_proj, had_dim=head_dim, output=True)  # 对v进行(ic,head,head_dim)拆分然后最最后一个维度使用H变换, 与
    apply_exact_had_to_linear(o_proj, had_dim=-1, output=False)  # W_o @ Q  对o_proj进行在线H变换(和输入在线H做对应)
...
def rotate_model(model, args):
    ...
    model_type = model_utils.model_type_extractor(model)
    rotate_embeddings(model, Q)  # W_embeddin(b,ic) @ Q
    rotate_head(model, Q)  # W_head @ Q
    utils.cleanup_memory(False)
    layers = model_utils.get_transformer_layers(model, model_type=model_type)
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        rotate_attention_inputs(layers[idx], Q, model_type)  # Wq(oc,ic) @ Q , Wk @ Q, Wv @ Q # 等价于(XQ @ (Wq @ Q)^T) = (XQQ^T Wq^T)
        ...
        """"""
        rotate_mlp_input(layers[idx], Q, model_type)  # W_mlp @ Q
        rotate_mlp_output(layers[idx], Q, model_type, args.qwen2)  # Q^T @ W_mlpo(residual匹配) , W_mlpo @ Q11122(和输入动态H相对应)
        rotate_ov_proj(layers[idx], model_type, num_heads, head_dim)  # 对V进行输出的H变换: Wv^T(ic,head,head_dim) @ Q^{hd}  O进行输入的H变换: W_o @ Q
    ...
    def __init__(self, func, config, *args, **kwargs):
        ...
        if kwargs is not None:
            ...
            self.k_quantizer.configure(
                bits=self.k_bits, groupsize=-1, sym=self.k_sym, clip_ratio=self.k_clip_ratio  # we put -1 to be toke-wise quantization and handle head-wise quantization by ourself
            )

    def forward(self, *args, **kwargs):
        q, k = self.func(*args, **kwargs)
        dtype = q.dtype
        q = hadamard_transform(q.float(), scale=1 / math.sqrt(q.shape[-1])).to(dtype)  # q和k都进行一次H变换,即 q @ Q ,  k @ Q
        k = hadamard_transform(k.float(), scale=1 / math.sqrt(k.shape[-1])).to(dtype)
        (bsz, num_heads, seq_len, head_dim) = k.shape

        if self.k_groupsize == -1:  # token-wise quantization 如果k不分组就使用per-token量化
            token_wise_k = k.transpose(1, 2).reshape(-1, self.config.hidden_size)  # (bh,l,c) -> (bh,c,l) -> (bhc,l)
            # token_wise_k = k.transpose(1, 2).reshape(-1, 4096) # qwen2, hiddensize = 3584, can not directly reshape for pertoken, to improve in the future
            self.k_quantizer.find_params(token_wise_k)
            k = self.k_quantizer(token_wise_k).reshape((bsz, seq_len, num_heads, head_dim)).transpose(1, 2).to(q)

        else:  # head-wise quantization # 使用per-head的量化
            per_head_k = k.view(-1, head_dim)  # (bh,l,c) -> (bhl,c)
...
def add_qk_rotation_wrapper_after_function_call_in_forward(module, function_name, *args, **kwargs):
    ...
    wrapper = monkeypatch.add_wrapper_after_function_call_in_method(module, "forward", function_name, functools.partial(QKRotationWrapper, *args, **kwargs))
