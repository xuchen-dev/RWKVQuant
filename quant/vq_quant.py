...
from quant.uniform_quantizers import SymmetricUniformQuantizer
...
def get_assignments(X, centroids, chunk_size=None, H_inv_diag=None):
    ...
    if H_inv_diag is None:
        ...
        assert H_inv_diag.shape[0] == 1 and H_inv_diag.shape[1] == X.shape[1] and H_inv_diag.shape[2] == X.shape[2], f"{H_inv_diag.shape, X.shape}"
        H_inv_diag = H_inv_diag.unsqueeze(2)
    ...
    for X, H_inv_diag in zip(X_chunks, H_inv_diag_chunks):
        ...
        assignments.append(dist.argmin(-1))
    assignments = torch.concat(assignments, dim=1)

    return assignments, dist  # G x N
...
def vq_quantize(X, quantizer, H_inv_diag=None, centroids=None):
    ...
    idx, dist = get_assignments(X, centroids, chunk_size=quantizer.assignment_chunk_size, H_inv_diag=H_inv_diag)  # G x N
...
def vq_quantize_mul(X, quantizer, mask=None, centroids=None):
    ...
    if centroids is None:
        centroids = quantizer.all_centroids[-1]  # G x K x D
    idx, dist = get_assignments_mul(X, centroids, chunk_size=quantizer.assignment_chunk_size, mask=mask)  # G x N
...
def kmeans_m_step_3(
    ...
    if H_inv_diag is None:
        ...
        norm = 1.0 / torch.clip(torch.einsum("gnk,nd->gkd", assignments_expanded, H_inv_diag[0]), min=1e-10)
        clusters_for_centroid = torch.einsum("gnd,nd,gnk,gkd->gkd", X, H_inv_diag[0], assignments_expanded, norm)
...
def kmeans_vq(
    ...
    for iter in range(iters):
        # E-step
        assignments, dist = get_assignments(X, centroids, chunk_size=assignment_chunk_size, H_inv_diag=H_inv_diag)
    ...
    # idx = dist.abs().min(-1)[1]
    # min = 64
    # for group in idx:
    #     freq = torch.tensor( [ torch.sum(torch.eq(group, i)) for i in range(8) ])
    #     if freq.min() < min:
    #         min = freq.min()
    #         print(freq)

    dist = dist.abs().min(-1)[0].mean(-1).mean()
    # freq = torch.tensor( [ torch.sum(torch.eq(idx, i)) for i in range(8) ])
    # print(dist)
    # valid_book = len( dist.sum(1).min(-1)[1] )
    # dist_min =

    # print(valid_book, dist_min)
    return dist
...
def kpp_parallel_sampled(data: torch.Tensor, k: int):
    ...
    for data in split_data:
        init = torch.zeros((data.shape[0], k, data.shape[-1]), dtype=torch.float16).to(data.device)  # G, K, D
...
def kmeans_m_step_3_mul(
    ...
    if mask is None:
        norm = 1.0 / torch.clip(assignments_expanded.sum(1), min=1)
        clusters_for_centroid = torch.einsum("gnd,gnk,gk->gkd", X, assignments_expanded, norm)
    else:
        mask = mask.reshape(X.shape)
        # print("*****", assignments_expanded.shape,  mask.shape )

        score = assignments_expanded * mask
        norm = 1.0 / torch.clip(score.sum(1), min=1)
        clusters_for_centroid = torch.einsum("gnd,gnk,gk->gkd", X, score, norm)

        # norm = 1.0 / torch.clip(
        #     torch.einsum("gnk,nd->gkd", assignments_expanded, mask[0]), min=1e-10
        # )
        # clusters_for_centroid = torch.einsum(
        #     "gnd,nd,gnk,gkd->gkd", X, mask[0], assignments_expanded, norm
        # )

    centroids.copy_(clusters_for_centroid)


def get_assignments_mul(X, centroids, chunk_size=None, mask=None):
    """
    X: G x N x D
    centroids: G x K x D
    """
    if mask is None:
        mask = torch.ones(X.shape).to(X.device)  # 1

    mask = mask.reshape(X.shape).unsqueeze(-1)
    ...
    assignments = []
    for X, m in zip(X_chunks, masks):
        X = X.unsqueeze(2)  # G x N' x 1 x D

        # print(f"get dis X shape {m.shape} dis shape {(X - centroids).pow(2).shape}")
        dist = ((X - centroids).pow(2) * m).sum(-1)  # [2048, 512, 1, 1] - [2048, 1, 4, 1]
...
def kmeans_vq_mul(
    ...
    for iter in range(iters):
        # E-step
        assignments, dist = get_assignments_mul(X, centroids, chunk_size=assignment_chunk_size, mask=None)

        # M-step: gather all values for each centroid and compute means
        # Centroids is shape G x D x K; assignments is shape G x N
        kmeans_m_step_3_mul(centroids, n_centroids, assignments, X, mask=mask)
...
def mahalanobis_init(X, n_centroids, mask=None):
    """
    X: G x N x D
    centroids: G x K x D
    """
    ...
    if mask is not None:
        Lambda = torch.linalg.inv(Sigma) * mask
    else:
    ...
    idx = sorted_dists[:, idx].unsqueeze(-1).expand(-1, -1, vq_dim)  # G x K --> G x K x 1 --> G x K x D
...
def quantize_centroids(centroids, bitwidth, per_codebook=True):
    ...
    if not per_codebook:
        centroids_ = centroids.contiguous().view(1, -1)
    else:
...
class VQQuantizer(nn.Module):
    ...
    def get_groupsize(self, X, groupsize):
        if len(X.shape) == 3:
            ...
            if X.shape[1] == 1:
                assert X.shape[2] % groupsize == 0
                self.groups_per_column = 1
                return groupsize
            else:
        ...
        if groupsize % X.shape[1] != 0:
            print(f"Requested groupsize {groupsize} doesn't fit tensor shape[0] {X.shape[0]}. " f"Upscaling to {int(np.ceil(groupsize / X.shape[0]) * X.shape[0])}")
    ...
    def find_params(self, X: torch.Tensor, weight=True, H_inv_diag=None, mask=None):
        assert weight
        assert len(X.shape) == 2
        ...
        if self.kmeans_init_method == "cdf":
            ...
            centroids = mahalanobis_init(X, self.n_centroids)  # 2bit
        else:
        ...
        if self.quantize_during_kmeans and self.codebook_bitwidth is not None:
            extra_args = dict(codebook_bitwidth=self.codebook_bitwidth, per_codebook=self.quantize_per_codebook)
        ...
        if self.codebook_bitwidth is not None and not self.quantize_during_kmeans:
            quantize_centroids(centroids, self.codebook_bitwidth, per_codebook=self.quantize_per_codebook)

        self.all_centroids.append(centroids)
        return dist

    def find_params_mul(self, X: torch.Tensor, weight=True, H_inv_diag=None, mask=None):
        assert weight

        X = X.reshape(self.groups_per_column, -1, self.vq_dim)  # G x N x D  [1, 64, 1]
        if H_inv_diag is not None:
            H_inv_diag = H_inv_diag.reshape(1, -1, self.vq_dim)  # 1 x N x D
            if self.rows_per_group > 1:
                H_inv_diag = H_inv_diag.tile(1, self.rows_per_group, 1)
    ...
    def blockwise_normalize_data(
        ...
        if self.vq_scaling_blocksize > 0:
            ...
            if vq_scaling_domain == "log":
                ...
                self.log_scales[torch.abs(self.scales) < 1.0e-8] = 0.0  # don't scale zeros, keep them as it is
