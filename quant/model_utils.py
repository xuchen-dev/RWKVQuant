...
def get_llama(model_name, hf_token):
    ...
    model = transformers.LlamaForCausalLM.from_pretrained(model_name, torch_dtype="auto", use_auth_token=hf_token, low_cpu_mem_usage=True)
    model.seqlen = 2048
...
def get_qwen(model_name, hf_token):
    ...
    torch.nn.init.uniform_ = skip
    torch.nn.init.normal_ = skip
    model = transformers.Qwen2ForCausalLM.from_pretrained(model_name, torch_dtype="auto", use_auth_token=hf_token, low_cpu_mem_usage=True)
    model.seqlen = 2048
    logging.info("---> Loading {} Model with seq_len: {}".format(model_name, model.seqlen))
    return model
...
def capture_layer_io(model_type, layer, layer_input):
    ...
    if model_type in (LLAMA_MODEL, QWEN_MODEL):
        ...
        captured_inputs = {"k_proj": [], "out_proj": [], "fc1": [], "fc2": []}  # q_proj, v_proj has the same input as k_proj
        captured_outputs = {
            "v_proj": [],
        }
        for name in captured_inputs.keys():
